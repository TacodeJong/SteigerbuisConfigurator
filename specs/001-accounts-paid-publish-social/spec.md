...
**Pay-per-use gates** (`app_settings.feature_gates`, Beheer → Betaalde functies): `full_print`, `copy_order_list`, `bom_print`, `viewport_print`, `download_model`. Gate on → subscription covering the feature **or** durable grant **or** (print/copy) `export_pack`.
...
- **FR-007c**: System MUST gate **Bestellijst kopiëren naar klembord** to `copy_order_list` when `feature_gates.copy_order_list` is on (default on). Access = active subscription covering the feature **OR** durable model grant **OR** account-wide export pack. Stuklijst print MAY be gated via `feature_gates.bom_print` (default off). **3D-weergave printen** MUST use dedicated `feature_gates.viewport_print` (default off) — not `bom_print`.
